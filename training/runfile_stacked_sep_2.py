...
model=load_model('models/model_stacked_sep_2')

opt = keras.optimizers.Adam(learning_rate=0.001)
model.compile(loss = 'mean_squared_error', optimizer = opt, metrics = 'mse')

mc = ModelCheckpoint('model_stacked_sep_2.h5', monitor='val_loss', mode='min', save_best_only=True, verbose=1)
es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=100)

history = model.fit(x = [features_train[:,:,:,:,0:1], features_train[:,:,:,:,1:2], features_train[:,:,:,:,2:3], features_train[:,:,:,:,3:4]], y = labels_train, validation_data = ([features_val[:,:,:,:,0:1], features_val[:,:,:,:,1:2], features_val[:,:,:,:,2:3], features_val[:,:,:,:,3:4]], labels_val), epochs = 250,
                    batch_size = 1, verbose = 2, callbacks=[es, mc], shuffle = False)

array_hist = numpy.array(list(history.history.values())).transpose()
numpy.save('history_stacked_sep_2.csv', array_hist)